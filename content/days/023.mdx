...
> **Note:** Many frontier models (GPT-4o, Claude 3.7 Sonnet, Gemini 2.5 Pro) now perform internal chain-of-thought automatically without explicit prompting. However, _explicit_ CoT prompting via XML tags remains best practice for auditable, controllable reasoning in production systems.
...
#### Extended Thinking (Inference-Time Compute Scaling)

A third paradigm, pioneered by OpenAI's o1/o3/o4-mini series and Anthropic's Claude 3.7 Sonnet with extended thinking mode, scales reasoning by allocating additional **inference-time compute** rather than relying on prompt structure.

- The model reasons internally for seconds or minutes before producing output.
- You do not craft a CoT prompt — you set a "thinking budget" (tokens or seconds) and the model self-organizes its reasoning.
- **Key models:** OpenAI o3, o4-mini; Claude 3.7 Sonnet (extended thinking mode); Gemini 2.0 Flash Thinking; DeepSeek-R1.
- **Trade-off:** Dramatically better on hard logic/math; dramatically more expensive and slower. Not suitable for latency-sensitive paths.
- **Your gateway abstraction must account for this:** these models have different API parameters (`reasoning_effort`, `thinking_budget`) compared to standard chat completions.
...
- **Using Extended Thinking for Simple Queries:** Routing a "What's the weather?" question to an o3-class reasoning model is wasteful. Implement **Complexity Routing** (see Day 21) to reserve inference-time compute models for tasks that actually need them.
