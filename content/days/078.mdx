...
3. **Metadata Pre-Filtering:** The backend constructs the vector database query using a hardcoded metadata filter. Most modern vector DBs (such as **Pinecone v3+** metadata filtering or **Qdrant** payload-based filtering) support `$in` or `$contains` operators that run efficiently via inverted indexes before the dense vector calculation occurs.
