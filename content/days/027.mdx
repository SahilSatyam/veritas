...
- **Student:** GPT-5.4-mini / Llama 4 Scout (Production Runner).
- **Judge:** GPT-5.5 / Claude Opus 4.8 / Prometheus-2 (Offline Auditor).
- **Why?** The Judge is too slow/expensive for real-time users, but perfect for offline batch testing. Prometheus-2 offers an excellent open-source alternative designed specifically for LLM evaluation.
...
- _Rule:_ Cross-check. If using OpenAI for generation, use Anthropic for evaluation (and vice-versa), or deploy the **G-Eval** framework to structure the evaluation rubric with detailed chain-of-thought steps. This reduces the risk of shared "blind spots."
...
def evaluate_summaries(article: str, summary_a: str, summary_b: str, judge_client) -> Dict:
    ...
    response = judge_client.chat.completions.create(
        model="claude-3-7-sonnet", # The "Teacher" Model
        messages=[
...
**Day 28** covers **Embeddings & Vector Space**. Use **Embeddings**—high-dimensional vectors that represent _meaning_, not just characters to solve "The Synonym Gap" and improve user experience.
