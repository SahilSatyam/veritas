...
> **Tooling Note (2025+):** **Docker** remains the dominant standard, but **Podman** (from Red Hat) has emerged as a compelling rootless, daemonless alternative. Podman is fully OCI-compatible and largely Docker-CLI-compatible (`alias docker=podman`), but runs without a root-level daemon — a superior security posture for production AI workloads.
...
> **Rootless Alternative:** With **Podman**, containers run rootless by default — even the container daemon itself runs as a non-root user. This provides defense-in-depth: if the container process is compromised, the attacker gains no host privileges. The Dockerfile format is identical; just replace `docker` with `podman` in all commands.
...
- **Rootless Alternative**: _Podman Documentation_ (docs.podman.io) - Daemonless, rootless container engine; drop-in Docker replacement with superior default security.
