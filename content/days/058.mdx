...
Advanced attackers use **BEAST attacks** (a sophisticated prompt-level poison), "Clean Label Attacks", or "Latent Backdoors" designed to merge into the spectral distribution of the target class. Benchmark suites like the **TrojAI (Trojan AI)** project from NIST and IARPA serve as critical evaluation standards for evaluating trojan detection tools under these advanced threats. Defense is never static.
