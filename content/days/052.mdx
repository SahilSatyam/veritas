...
We will use **DiCE (Diverse Counterfactual Explanations)**, **CARLA (Counterfactual And Recourse Library)**, or **Alibi**, but wrapped in a strict validation layer. Raw library outputs are often dangerous (e.g., suggesting "Change Gender"). CARLA is a particularly modern choice as it provides a unified interface across multiple counterfactual methods and datasets, making it easy to swap algorithms without rewriting the constraint layer.
...
- **Library:** [CARLA](https://github.com/carla-recourse/CARLA) – Counterfactual And Recourse Library; a benchmarking framework providing a unified API across many CF methods.
