...
- **The Perplexity.ai Pattern**: The dominant citation UI pattern in production systems (popularized by Perplexity AI and now adopted by Bing Copilot, Google AI Overviews, and enterprise search products) renders inline superscript source numbers (`[1]`, `[2]`) linked directly to source URLs. Each claim is immediately hoverable to preview the source excerpt. When designing your citation UI, adopt this inline-citation pattern over footnote-only or document-list approaches—users have been conditioned to expect it, and it maximally surfaces trust signals at the point of consumption.
