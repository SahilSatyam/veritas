...
- **The Cost Trap:** A standard API call (e.g., getting the weather) costs effectively zero. An LLM API call can cost $0.002 to $0.15 depending on complexity (frontier models like GPT-4o, Gemini 2.5 Pro, and Claude 3.7 Sonnet are priced per million tokens, representing a massive shift in unit economics). A `while(true)` loop in GenAI is a bankruptcy vector.
