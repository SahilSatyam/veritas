...
We utilize the **LLM-as-a-Judge** paradigm. Since linguistic correctness is hard to measure with regex, we employ a stronger model (e.g., GPT-4o or a specialized fine-tuned evaluator) to grade the outputs of the production model.
...
def run_evaluation_pipeline(test_dataset: dict):
    ...
    Args:
        test_dataset: Dict containing 'user_input', 'response', 'retrieved_contexts', 'reference'
                      (In reality, 'response' comes from the model being tested)
    """

    # 1. Convert to RAGAs EvaluationDataset (v0.2+ API)
    ...
    # 2. Run Ragas Evaluation (v0.2+ API uses keyword argument)
...
- **Bias in the Judge**: Be aware that "LLM-as-a-Judge" models (usually GPT-4o) have their own biases. They tend to favor longer, more verbose answers (length bias) even if they are less concise.
...
- **Framework**: _Ragas: Automated Evaluation of Retrieval Augmented Generation_ (Es et al., 2023). Note: The library has migrated to a v0.2+ API using `EvaluationDataset` and class-based metrics—check the official docs for the latest import paths.
...
- **Tool**: _Langfuse_ (open-source LLM observability platform with native RAGAs integration for tracking evaluation scores over time).
