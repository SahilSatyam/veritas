...
2. **Evaluating Vendor Models:** You cannot probe closed APIs (GPT-4o, Gemini 2.5 Pro) deeply. This is a strategic argument for **Open Weights** models (Llama 3.3, Mistral, Qwen2.5) in high-compliance sectors. If you can't probe the weights, you can't verify the reasoning.
...
4. **Advanced — SAE Probing:** Use Sparse Autoencoders (SAEs) to first decompose polysemantic neurons into monosemantic features, then probe those features for truth-directional signals. This 2024–2025 technique from Anthropic research (see Neuronpedia) provides far more interpretable probes than raw activation probing.
...
- **Technique:** SAE-based probing – Use Sparse Autoencoders to decompose activations into interpretable features before probing; see Anthropic's Scaling Monosemanticity (2024) and [Neuronpedia](https://neuronpedia.org).
