...
- **System Prompt Caching:** Anthropic's Prompt Caching feature (and similar mechanisms on other platforms) allows you to mark your large `System Prompt` as a cacheable prefix. Subsequent API calls that reuse the same system prompt hit the cache, reducing both latency and cost by up to 90% on the cached portion. This makes the RTF pattern doubly valuable: a well-structured, stable system prompt is also an optimal caching target. Mark it with `cache_control: {"type": "ephemeral"}` in the Anthropic SDK.
...
**Next Up:** [Day 23: Prompt Engineering II: Reasoning (CoT & ReAct)](/day/023)
