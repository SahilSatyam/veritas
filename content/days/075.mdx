...
- **Vision-Language Models (VLMs):** Production-grade frontier models like **GPT-4o Vision**, **Gemini Vision**, or open-weights champions like **LLaVA-1.6**, **Qwen-VL**, and **InternVL2** accept both text and image tokens natively, capable of deep reasoning about visual relationships, spatial layouts, and chart topologies.
- **Image Embeddings vs. Image Summaries:**
  - _Embeddings (CLIP/SigLIP):_ Mathematical representations of an image in a shared latent space. While foundational, CLIP is no longer the primary production approach for analytical RAG due to its inability to reason about dense visual details.
  - _Summaries (VLM Extraction):_ Using a highly capable VLM to translate pixel data into structured text (e.g., Markdown tables) to allow standard, high-precision semantic search.
