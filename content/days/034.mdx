...
- **Step-Back Prompting**: A Google DeepMind technique that instructs the LLM to first generate an abstract, high-level "step-back" question (e.g., rewrite "What is the boiling point of water at 8,000 feet?" to "What are the physics of boiling point and pressure?"). Retrieving against this abstracted query improves recall for highly specific or technical user questions.
- **Adaptive RAG**: A routing strategy that classifies each incoming query and selects the appropriate retrieval strategy dynamically—direct retrieval for simple factual lookups, single-step retrieval for focused questions, and multi-hop decomposition for complex comparative queries. This avoids the cost of decomposition on every query.
...
- **Paper**: _Step-Back Prompting_ (Google DeepMind) — abstracts the user query to a higher-level principle before retrieval, dramatically improving recall on technical and scientific questions.
- **Paper**: _Adaptive-RAG: Learning to Adapt Retrieval-Augmented Large Language Models through Question Complexity_ — introduces learned routing to select the right retrieval strategy per query.
