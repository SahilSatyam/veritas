...
1.  Install `uv` (`curl -LsSf https://astral.sh/uv/install.sh | sh`) or `poetry`.
2.  Generate a lock file (`uv lock` or `poetry lock`).
3.  Delete your `venv` and reinstall only from the lock file to verify it works (`uv sync` or `poetry install`).
