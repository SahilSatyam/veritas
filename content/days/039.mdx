...
Vector databases are the long-term memory of AI applications, but they behave differently than relational databases at scale. A common production outage occurs when a team migrates from a prototype (10k vectors) to production (100M vectors) and realizes that **filtered vector search**—e.g., "Find similar documents _owned by User A_"—causes latency to spike from 20ms to 2000ms. This is the "Metadata Explosion." This post dissects the mechanics of Approximate Nearest Neighbor (ANN) search under constraints, focusing on HNSW parameter tuning, indexing strategies for metadata in modern engines (like **Qdrant** as the primary open-source leader, or **Pinecone** v3+ utilizing gRPC and serverless namespaces), and the critical security architecture required for rigorous multi-tenancy.
...
- **pgvector 0.7+ Improvements**: For relational databases, pgvector 0.7+ has significantly advanced HNSW indexing directly inside PostgreSQL, matching dedicated vector DB speeds for many use cases.
...
**Recommendation**: Use Logical Separation with strict Metadata Indexing (such as Qdrant payload-based filtering or pgvector 0.7+ indexes) for standard SaaS. Use Namespaces (like Pinecone's serverless namespaces with gRPC clients) only for VIP tenants requiring physical isolation.
