...
3. **Phase 3: Self-Host (Open-Weights on Cloud Compute):** Escalate to self-hosting a model (e.g., vLLM serving Llama 4 on AWS SageMaker) _only_ when token volume crosses the mathematical break-even point, or if data privacy regulations explicitly prohibit routing data to third-party sub-processors.
