...
Imagine you upgrade from `gpt-3.5-turbo` to a fine-tuned Llama 4 model to save costs. The Llama model passed all your unit tests. You flip the switch. Immediately, users start complaining that the chatbot is refusing to answer benign questions because the fine-tuning data was too safety-heavy. You have to roll back, but you've already eroded user trust.

Shadow deployment allows you to run that Llama 4 model alongside GPT-5.5-instant for a week. You would have seen the refusal spike in the logs _before_ a single user was affected.
...
**Feature Flagging for Dynamic Routing**
Rather than hard-coding routing logic, production teams use **feature flag platforms** to control which users hit which model version at runtime—without a code redeploy.

- **LaunchDarkly:** Enterprise-grade feature management with targeting rules, percentage rollouts, and kill switches ideal for progressive canary traffic routing.
- **Flagsmith:** Open-source alternative to LaunchDarkly, self-hostable, with a Python SDK that integrates natively into inference services.
- _Pattern:_ A flag named `model_version` routes `control` group to the live model and `experiment` group to the shadow model. This makes shadow promotion as simple as flipping a flag in a UI.
...
- **Feature Flags:** LaunchDarkly (enterprise) or Flagsmith (open-source, self-hostable) for managing traffic routing dynamically without redeployment.
- **Model Observability:** Arize AI, WhyLabs, or Evidently AI for monitoring model drift in shadow mode.
