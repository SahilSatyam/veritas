...
1. **Specialized (Qdrant, Pinecone, Weaviate, Chroma):** Built from scratch for vectors. Qdrant has emerged as the dominant Rust-based open-source production leader, while Pinecone serves as a standard managed SaaS option (using the modern v3+ class-based `Pinecone()` constructor rather than the deprecated `pinecone.init()`). Weaviate offers rich graph-like structural queries, and Chroma is the standard for local in-memory prototyping.
