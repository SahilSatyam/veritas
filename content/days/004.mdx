...
- **Property-Based Testing**: `hypothesis` (Auto-generates adversarial edge cases; the practical implementation of Section 3's theory).
...
### Step 5: Property-Based Testing with `hypothesis`

Section 3 introduced property-based testing conceptually. The `hypothesis` library implements it: instead of hand-crafting edge cases, it *auto-generates* thousands of adversarial inputs.

```python
# tests/test_preprocessing_hypothesis.py
import pytest
import pandas as pd
import numpy as np
...
# Define a strategy: generate DataFrames with arbitrary 'amount' and 'category' values
@given(
    df=data_frames([
        column('amount', elements=st.floats(allow_nan=True, allow_infinity=True)),
        column('category', elements=st.one_of(st.none(), st.text(max_size=20))),
        column('id', elements=st.integers(min_value=1))
    ])
)
@settings(max_examples=500)  # Hypothesis will generate 500 random DataFrames
def test_normalize_never_produces_infinity(df):
    """Property: The output MUST never contain infinite values, for ANY input."""
    try:
        result = normalize_transactions(df)
        # The key property: no infinite values regardless of input
        if 'log_amount' in result.columns:
            assert not np.isinf(result['log_amount']).any(), \
                "Infinite values found in log_amount"
    except (ValueError, KeyError):
        pass  # Graceful failure on structurally invalid input is acceptable
```

This test does in 10 lines what would take hundreds of hand-written test cases. `hypothesis` deliberately explores boundary values: 0.0, `-inf`, `NaN`, very large floats, etc.
...
- **Property-Based Testing**: _Hypothesis Documentation_ (hypothesis.works) - The definitive tool for auto-generating adversarial test cases; pairs directly with pytest.
