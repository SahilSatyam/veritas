...
**Expanding the Toolkit (Beyond SHAP):**

- **Captum (PyTorch-native):** If your model is a neural network built in PyTorch, Captum provides integrated gradients, layer attribution, and TCAV — all natively inside the PyTorch ecosystem without the model-agnostic overhead of SHAP's KernelExplainer.
- **MAGE (Model-Agnostic Global Explanations):** A newer framework for counterfactual and contrastive explanations, particularly useful for non-tree-based models where "what would need to change for a different outcome" is more actionable than a raw importance score.
...
- **Library:** [Captum](https://captum.ai/) (PyTorch-native interpretability — integrated gradients, layer attribution, TCAV).
- **Library:** [LIME (GitHub)](https://github.com/marcotcr/lime) (Local Interpretable Model-agnostic Explanations — still useful for quick local approximations).
