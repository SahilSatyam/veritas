...
**2. The Guardrail API**
In production, we use dedicated security APIs and frameworks like **Guardrails AI** (leveraging modern v0.5+ API runner patterns), **NVIDIA NeMo Guardrails** (for programmable dialog rails), **LLM Guard** (a robust open-source security toolkit), or **LakeraGuard** / **Azure Content Safety** to scan and score prompt inputs before they ever hit the LLM.
...
- **Paper**: _Not what you’ve signed up for: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection_ (Greshake et al.).
