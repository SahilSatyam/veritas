...
Be aware that using a small embedding model to judge a large LLM can be limiting. If the embedding model doesn't understand the nuance of "negligence," it might pass a bad translation. For high-stakes domains (legal, medical), consider using a stronger model (e.g., GPT-4o, Claude 3.7 Sonnet, or a finetuned BERT) as the judge, despite the cost.
...
- **Promptfoo:** A popular open-source CLI and library for prompt testing and evaluation. Supports running test suites across multiple models, comparing outputs, and integrating into CI/CD pipelines. Ideal for teams wanting a lightweight, code-first evaluation harness.
- **Braintrust:** Production-grade LLM evaluation platform with experiment tracking, A/B testing, and a dataset management UI for iterating on prompt quality over time.
- **LLM-as-a-Judge Patterns:** Using GPT-4o or Claude 3.7 Sonnet to grade outputs. See the "G-Eval" paper for the theoretical foundation.
