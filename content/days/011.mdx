...
- **Brand Risk:** Public demonstrations of bias (e.g., facial recognition failures) are difficult to recover from. The COMPAS recidivism algorithm audit (Angwin et al., ProPublica) remains the canonical example: the system was nearly twice as likely to falsely flag Black defendants as future criminals compared to white defendants, despite equivalent predictive accuracy on aggregate metrics.
- **Regulatory Mandate:** Under the EU AI Act (in force August 2024), high-risk AI systems in credit, employment, and law enforcement are legally required to undergo bias audits before deployment and on an ongoing basis.
...
- **Audit:** Angwin et al. (ProPublica, 2016). _Machine Bias: There's Software Used Across the Country to Predict Future Criminals. And It's Biased Against Blacks._ (The COMPAS audit — essential reading for real-world consequences of biased risk scoring.)
